...
class MockPaymentGateway implements PaymentGateway
{
    ...
    public function capture(Payment $payment, PassengerPaymentMethod $method): GatewayResult
    {
        return match ($this->scenarioFor($method)) {
            self::CARD_DECLINED => GatewayResult::declined(self::NAME, 'Card declined by issuer.'),
            self::CARD_ERROR => GatewayResult::error(self::NAME, 'Gateway temporarily unavailable.'),
            default => GatewayResult::success(self::NAME, 'cap_mock_'.bin2hex(random_bytes(12))),
        };
    }
