        ...
        .catch((reason: unknown) => {
          ...
          setError(
            reason instanceof Error
              ? reason.message
              : "Could not retrieve outstanding payments.",
          );
          setItems([]);
        });
    ...
    <PaymentScreen title="Outstanding payments" subtitle="Resolve previous ride payments securely">
      {error ? (
        <PaymentCard style={styles.clearCard}>
          <Ionicons name="alert-circle-outline" size={24} color={paymentTheme.danger} />
        ...
        items.map((item) => (
          <PaymentCard key={item.rideId}>
            ...
            <View style={styles.action}>
              <PaymentButton
                label="Choose card and pay"
                icon="card-outline"
                onPress={() => router.push({ pathname: "/payments/cards", params: { mode: "retry", rideId: item.rideId, amount: String(item.amount) } })}
              />
            </View>
          </PaymentCard>
        ))
...
const styles = StyleSheet.create({
  ...
  errorText: { color: paymentTheme.danger, flex: 1, fontSize: 13, fontWeight: "700", lineHeight: 19 },
